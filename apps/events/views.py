...
def eventDetail(request, slug):
    ...
    games = (
        get_object_or_404(Event,
                          slug=slug).games.get_queryset().order_by('day')
    )
    ...
    for game in total_games:
        ...
        days_format.append(
            get_weekday(day.strftime("%A")) + " " + str(day.day) + " " +
            get_month(day.month)
        )
    ...
    days_formats_and_games = [
        {
            "format_day": i[0],
            "game": i[1]
        } for i in zip(days_format, total_games)
    ]
